---
layout: project_page
permalink: /
title: "Pet Caring"
authors:
  - Chaeeun Song*
  - Yeeun Yang
affiliations:
  - Soongsil University
site: "http://petcaring.site/"
---

# Pet Caring

<div align="center">

[![Visit Site](https://img.shields.io/badge/Visit-Site-green)](http://petcaring.site/)

<br>


</div>

---

## 소개

**강아지·고양이 영양 정보를 AI 뭉치에게 물어보세요!**

- 강아지·고양이에게 일반식과 영양제를 줄 때,  
  **AI 뭉치**를 이용하면 영양성분 정보를 빠르고 간편하게 얻을 수 있습니다.
- 어디 놀러갈지 고민될 때는 **장소 추천 서비스**를,  
  궁금한 점은 **커뮤니티**에서 질문해보세요.

---

## 주요 서비스

### 🗺️ 장소 추천

<div align="center">
  <img src="images/placeIMG.png" alt="장소 추천 서비스" width="60%" style="border-radius: 10px;"/>
</div>

- 광고/매크로 없이 우리집 반려동물과 어울리는 장소를 추천합니다.
- 카페·음식점 위주가 아닌, 관광지(프로방스, 팜랜드 등)도 포함!
- 트리 기반 추천 알고리즘으로 **반려동물 출입 가능 장소**만 안내합니다.

---

### 💬 커뮤니티

<div align="center">
  <img src="images/commuIMG.png" alt="커뮤니티" width="60%" style="border-radius: 10px;"/>
</div>

- 사용자끼리 자유롭게 질문·답변을 주고받는 **반려동물 커뮤니티**입니다.
- 강아지와 고양이에 대해 궁금한 점을 함께 나눌 수 있습니다.

---

### 🤖 AI 뭉치

<div align="center">
  <img src="images/AIimg.png" alt="AI 뭉치" width="60%" style="border-radius: 10px;"/>
</div>

- 저장된 반려동물 정보를 기반으로 **맞춤형 영양 정보**를 제공합니다.
- 이름, 품종, 비만 등 질병 정보까지 반영하여 **개별화된 답변**을 제안합니다.
- 음식 영양성분표와 손질법, 섭취 권장 여부까지 안내합니다.

---

## 빠른 체험

<div align="center" style="margin-top: 2em;">
  <a href="http://petcaring.site/" target="_blank">
    <img src="https://img.shields.io/badge/펫케어링-바로가기-3c7be4?style=for-the-badge" alt="펫케어링 사이트 바로가기" />
  </a>
  &nbsp;
  <a href="http://43.202.9.123/html/chat/chatbot.html" target="_blank">
    <img src="https://img.shields.io/badge/뭉치AI-체험하기-0d0e3f?style=for-the-badge" alt="뭉치AI 체험하기" />
  </a>
</div>

---

## FAQ

<details>
  <summary><b>Q. 질문/상담 내용은 안전한가요?</b></summary>
  <ul>
    <li>네, 모든 상담 내용은 안전하게 암호화되어 저장되며, 외부에 공개되지 않습니다.</li>
  </ul>
</details>

<details>
  <summary><b>Q. AI 챗봇이 실제 사람처럼 상담해주나요?</b></summary>
  <ul>
    <li>뭉치는 실제 상담 전문가의 대화 데이터를 학습하여, 공감과 조언을 제공합니다.</li>
  </ul>
</details>

<details>
  <summary><b>Q. 서비스 이용에 비용이 드나요?</b></summary>
  <ul>
    <li>모든 서비스는 무료로 제공됩니다.</li>
  </ul>
</details>

---


<div align="center" style="color:#888; font-size:0.95em;">
서울시 동작구 상도로 369(상도동, 숭실대학교) <br>
Copyright by La plata. All Rights Reserved.
</div>
